...
export default function ModalContent({ salon }: { salon: any }) {
  ...
  return (
    <div className="p-6">
      {faceDetection ? (
       "WebcamDemo is not defined"
      ) : (
        <div
          className="border border-black w-[6.5rem] p-2 rounded-md cursor-pointer"
          onClick={() => {
        ...
        >
          ...
          <p
            className=" mt-2 text-[0.4rem] text-justify"
      ...
      {/* select option */}
      <select
        className="block w-full mt-1 p-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer"
        onChange={(e) => {
