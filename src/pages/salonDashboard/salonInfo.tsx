...
export default function SalonInfo() {
  ...
  useEffect(() => {
    ...
    fetch(`${baseUrl}/api/salon/one/{id}?id=${loadFromLocalStorage("id")}`, {
      ...
      .then((data) => {
        ...
        if (
          !servicesList.find(
            (service: { name: string }) => service.name === "Skin Care"
          )
        ) {
        ...
        if (
          !servicesList.find(
            (service: { name: string }) => service.name === "Spa Service"
          )
        ) {
        ...
        if (
          !servicesList.find(
            (service: { name: string }) => service.name === "Special Service"
          )
        ) {
  ...
  return (
    <div>
      <h1
        className="text-center font-bold text-[1.5rem] text-[#d18d32]"
        style={{
          textShadow:
        ...
        <div className="flex flex-col  space-y-4">
                  ...
                  <tbody>
                    {salonInfo.servicesList.map((service, index) => {
                      if (
                        ...
                        return (
                          <tr key={index}>
                            <td className="border border-black pl-4">
                              {service.name}
                            </td>
                            <td className="border border-black pl-4">
                              {service.cost} BDT
          ...
          >
            {hairstyles.map((hairstyle, index) => (
              <SwiperSlide key={index}>
                ...
                >
                  <div
                    className={`bg-black bg-opacity-50 text-white text-center ${
                      hairstyleIndex === hairstyle.id ? "h-full" : ""
                    }`}
                  >
                    {hairstyle.name}
                    {hairstyleIndex === hairstyle.id && (
                      <div className="text-2xl mt-10">Selected</div>
                    )}
                  </div>
                </div>
              </SwiperSlide>
          ...
          <div className="flex flex-row items-center w-full space-x-4">
            ...
            <IconButton
              ...
              callback={() => {
                ...
                if (
                  salonInfo.servicesList.find(
                    (service) =>
                      service.name ===
                      hairstyles.find(
                        (hairstyle) => hairstyle?.id === hairstyleIndex
                      )?.name
                  )
                ) {
                  setAlertBox({
                ...
                setSalonInfo({
                  ...salonInfo,
                  servicesList: [
                    ...salonInfo.servicesList,
                    {
                      name: hairstyles.find(
                        (hairstyle) => hairstyle.id === hairstyleIndex
                      )?.name,
                      cost: hairstylePrice,
                    },
          ...
          <input
            onChange={(e) => {
              setSalonInfo({
                ...salonInfo,
                servicesList: salonInfo.servicesList.map((service) =>
                  service.name === "Skin Care"
                    ? { name: service.name, cost: parseInt(e.target.value) }
                    : service
                ),
              });
          ...
          <input
            ...
            value={
              salonInfo.servicesList.find(
                (service) => service.name === "Spa Service"
              )?.cost || ""
            }
            type="number"
          ...
          <input
            ...
            value={
              salonInfo.servicesList.find(
                (service) => service.name === "Special Service"
              )?.cost || ""
            }
            type="number"
          ...
          <IconButton
            icon={<FaSave size={20} className="ml-2"/>}
        ...
        {alert.message && (
          <div
            className={`text-center ${
              alert.isError ? "text-white" : "text-green-500"
            } text-sm ${
              alert.isError ? "bg-red-500" : "bg-lime-200"
            } me-2 rounded-md p-1 mt-6`}
            ref={(el) => {
