...
export default function BarberInfo() {
  ...
  return (
    <>
      {/* Barber Info */}
      <div className="flex flex-col  mx-auto md:w-[30rem] space-y-4 my-10">
        <h1 className="font-bold">Barber Information</h1>
        ...
        <select
          className="border border-black outline-none p-2 rounded-md mb-4"
          onChange={(e) => setBarberSkill(e.target.value)}
        >
        ...
        <h1 className="font-bold">Barber Availalibity:</h1>
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <span
              onClick={() => {
                if (barberAvailability.includes(day)) {
                  setBarberAvailability(
                    barberAvailability.filter((item) => item !== day)
                  );
                } else {
              ...
              key={index}
              className={`px-3 select-none block rounded-md ${
                barberAvailability.includes(day)
                  ? `bg-black font-bold`
                  : `bg-[#3b5997]`
              } text-white p-1 text-[0.8rem] cursor-pointer`}
            >
              {day.slice(0, 3)}
            </span>
          ))}
      ...
      {barberList.length === 0 ? (
        ...
        <div className="flex flex-col mx-auto w-[30rem] space-y-4 my-10">
          <h1 className="font-bold">Available Barbers</h1>
